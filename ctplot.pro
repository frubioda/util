...
pro ctplot,bild,x,y,xtitle=xtitle,ytitle=ytitle,charsize=charsize, $
           ...
           noylabel=noylabel,aspect=aspect,ctw=ctw, _extra=_extra
;
...
if not keyword_set(xstyle) then xstyle = 5.
if not keyword_set(ystyle) then ystyle = 5.
...
if (not keyword_set(noerase)) then $
   plot,x,y,xtitle=xtitle,ytitle=ytitle,charsize=charsize,/nodata, $
        xstyle=xstyle,ystyle=ystyle,title=title,xmargin=xmargin,ymargin=ymargin, _extra=_extra
;
;
; read out corners
;
